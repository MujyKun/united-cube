...
class Example:
    def __init__(self):
        self.kwargs: dict = {
            ...
            # not pass in a hook. (UCube.ucubeasync.UCubeClientAsync._start_loop_for_hook)
        }
    ...
    async def start(self):
        if not self.ucube_client:
            my_web_session = ClientSession()

            self.kwargs["web_session"] = my_web_session
        ...
        try:
            ...
            if self.ucube_client.cache_loaded:
                # will occur in a task and will not raise an exception here. Exception would be
                # raised in _wait_for_login
                await self.ucube_client._try_login()
                # wait for login or an exception to occur.
                # you may want to try-except _wait_for_login(), but this example will not contain it for readability
                await self.ucube_client._wait_for_login()

                # if we get to this line of code, that means the login was successful.
                # we can now start the hook loop again
                await self.ucube_client._start_loop_for_hook()
                # the code should now remain here unless another exception occurs
            ...
        except asyncio.exceptions.TimeoutError:
            print("It took too much time to log in.")
            ...
    ...
    async def on_new_notifications(self, notifications: List[UCube.models.Notification]):
        """
        ...
        for notification in notifications:
            ...
            if not post:
                ...
                # await self.ucube_client.fetch_post(notification.post_slug)
                ...
